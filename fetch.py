#!/usr/bin/env python3
"""
INVICTA-REIGN Market Terminal - data robot.

Reads config.json, fetches prices + news from free sources, writes data.json,
and (only if Telegram secrets are set) pushes alerts to your phone.

This is TRACK-AND-ALERT ONLY. It never logs in to any broker. It never trades.
You place every trade yourself, manually, on Groww.
"""
...
# ---- Mutual funds via mfapi.in (free, no key) ----
def fetch_funds(cfg):
    out = []
    dp = default_pct(cfg)
    for fnd in cfg.get("mutual_funds", []):
        code = str(fnd.get("scheme_code", "")).strip()
        row = {"name": fnd.get("name", "Fund"), "nav": None, "change_pct": None, "alert": False}
        if code.isdigit():
            try:
...
def fetch_commodities(cfg):
    ...
    for c in cfg.get("commodities_mandi", []):
        ...
        try:
            ...
            r = requests.get(f"https://api.data.gov.in/resource/{res}", params=params, timeout=25)
    ...
    for t in cfg.get("news_topics", []):
        try:
            url = ("https://api.gdeltproject.org/api/v2/doc/doc?query="
                   ...
                   + "&mode=ArtList&format=json&maxrecords=8&timespan=2d&sort=DateDesc")
            r = requests.get(url, timeout=30)
            for a in r.json().get("articles", []):
                out.append({"title": a.get("title", ""), "url": a.get("url", ""),
                            "domain": (a.get("domain", "news") or "news").replace("www.", ""),
                            "topic": t.get("label", ""), "seendate": a.get("seendate", "")})
        except Exception as e:
